...
class AddBusinessProfileViewController : UIViewController {
    ...
    @IBAction func addBtnClicked(_ sender: Any) {
        ...
        if !isCoverPhotoSelected {
            self.showSnack(messages: "Upload Cover Picture")
            return
        }
        if sTitle == "" {
        ...
        businessModel.businessCategory = Constants.BUSINESS_TYPE[businessTypePicker.selectedRow(inComponent: 0)]
        ...
        self.ProgressHUDShow(text: "Uploading Pictures")
        ...
        uploadFilesOnAWS(
            ...
            businessModel.profilePicture = downloadURL
            group.leave()
        }
        ...
        uploadFilesOnAWS(
            ...
            businessModel.coverPicture = downloadURL
            group.leave()
        }
        
        // Notify when all uploads are complete
        group.notify(queue: .main) {
            // Hide the loader once both uploads are finished
            self.ProgressHUDHide()
         
            self.addBusiness(businessModel: businessModel)
            
        }
    }
    
    func addBusiness(businessModel : BusinessModel) {
    ...
    CropViewControllerDelegate
{
    ...
    func cropViewController(_ cropView : CropViewController, didCropToImage image: UIImage, withRect _: CGRect, angle _: Int) {
        ...
        if cropView.title == "Profile Picture" {
            ...
            self.isCoverPhotoSelected = true
